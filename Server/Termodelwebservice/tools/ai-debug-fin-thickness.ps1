# AI debug Appartamento FIN wall-thickness coverage
param(
...
try {
  ...
  $coverageDetails = [System.Collections.Generic.List[string]]::new()
  $coverageFailures = 0
  $coverageChecked = 0
  ...
  if (Test-Path $modelArtifactPath) {
    ...
    $wallSides = @($primitives | Where-Object { $_.tipo -eq "Parete" -and $_.parte -eq "lati" })
    ...
    function Primitive-ProjectionRange($primitive,[double]$fx,[double]$fy,[double]$ux,[double]$uy,[double]$nx,[double]$ny) {
      $uValues = [System.Collections.Generic.List[double]]::new()
      $nValues = [System.Collections.Generic.List[double]]::new()
      foreach ($vertex in @($primitive.vertices)) {
        if ($null -eq $vertex -or @($vertex).Count -lt 2) { continue }
        $rx = [double]$vertex[0] - $fx
        $ry = [double]$vertex[1] - $fy
        $uValues.Add($rx*$ux + $ry*$uy)
        $nValues.Add($rx*$nx + $ry*$ny)
      }
      if ($uValues.Count -eq 0) { return $null }
      $minU = ($uValues | Measure-Object -Minimum).Minimum
      $maxU = ($uValues | Measure-Object -Maximum).Maximum
      $minN = ($nValues | Measure-Object -Minimum).Minimum
      $maxN = ($nValues | Measure-Object -Maximum).Maximum
      return [pscustomobject]@{
        MinU=[double]$minU; MaxU=[double]$maxU
        MinN=[double]$minN; MaxN=[double]$maxN
        SpanU=[double]($maxU-$minU); SpanN=[double]($maxN-$minN)
      }
    }

    function Interval-DistanceToZero([double]$minimum,[double]$maximum) {
      if ($minimum -le 0 -and $maximum -ge 0) { return 0.0 }
    ...
    foreach ($fin in $finNodes) {
      ...
      foreach ($wall in $wallLines) {
        ...
        if ($null -eq $sourceWall -or $candidate.Distance -lt $sourceWall.Distance) { $sourceWall = $candidate }
      }
      if ($null -eq $sourceWall) {
        $coverageFailures++
        $coverageDetails.Add("$finId ERROR source wall missing")
        continue
      }

      $ux = [double]$sourceWall.Dx
      $uy = [double]$sourceWall.Dy
      $nx = -$uy
      $ny = $ux
      ...
      foreach ($window in $windowSides) {
        ...
        $du = [Math]::Abs(($cx-$fx)*$ux + ($cy-$fy)*$uy)
        $dist = [Math]::Sqrt(($cx-$fx)*($cx-$fx)+($cy-$fy)*($cy-$fy))
        $score = $du*10.0 + $dist
        if ($null -eq $nearestWindow -or $score -lt $nearestWindow.Score) {
          $nearestWindow = [pscustomobject]@{ Score=$score; Distance=$dist; Primitive=$window }
        }
      }

      if ($null -eq $nearestWindow) {
        $coverageFailures++
        $coverageDetails.Add("$finId ERROR window primitive missing")
        continue
      }

      $windowRange = Primitive-ProjectionRange $nearestWindow.Primitive $fx $fy $ux $uy $nx $ny
      if ($null -eq $windowRange) {
        $coverageFailures++
        $coverageDetails.Add("$finId ERROR window projection missing")
        continue
      }

      $hostWall = $null
      foreach ($wallPrimitive in $wallSides) {
        $range = Primitive-ProjectionRange $wallPrimitive $fx $fy $ux $uy $nx $ny
        if ($null -eq $range) { continue }

        # La parete ospite deve essere lunga prevalentemente lungo l'asse
        # della linea SVG, contenere longitudinalmente il punto FIN e toccare
        # la linea di riferimento da cui il renderer estrude il suo spessore.
        if ($range.SpanU -lt [Math]::Max(0.20,$range.SpanN*1.25)) { continue }
        $uPenalty = Interval-DistanceToZero $range.MinU $range.MaxU
        $nPenalty = Interval-DistanceToZero $range.MinN $range.MaxN
        if ($uPenalty -gt 0.03 -or $nPenalty -gt 0.03) { continue }

        $score = $uPenalty*1000.0 + $nPenalty*10000.0 + $range.SpanN
        if ($null -eq $hostWall -or $score -lt $hostWall.Score) {
          $hostWall = [pscustomobject]@{ Score=$score; Primitive=$wallPrimitive; Range=$range }
        }
      }

      if ($null -eq $hostWall) {
        $coverageFailures++
        $coverageDetails.Add("$finId ERROR host wall primitive missing wall=$($sourceWall.Id)")
        continue
      }
      ...
      $alignment = if ($alen -gt 1e-12) { [Math]::Abs(($adx/$alen)*$ux + ($ady/$alen)*$uy) } else { 0.0 }

      $wallMin = [double]$hostWall.Range.MinN
      $wallMax = [double]$hostWall.Range.MaxN
      $windowMin = [double]$windowRange.MinN
      $windowMax = [double]$windowRange.MaxN
      $wallCenter = ($wallMin+$wallMax)*0.5
      $windowCenter = ($windowMin+$windowMax)*0.5
      $centerDelta = [Math]::Abs($windowCenter-$wallCenter)
      $revealMin = $wallMin-$windowMin
      $revealMax = $windowMax-$wallMax

      $coverageChecked++
      $ok = $alignment -ge 0.999 -and $centerDelta -le 0.0025 -and $revealMin -ge 0.005 -and $revealMax -ge 0.005
      if (-not $ok) { $coverageFailures++ }
  ...
  $lines = @(
    "TERMODEL_AI_APPARTAMENTO_FIN_THICKNESS_DEBUG_V1",
    "projectId=$projectId",
  ...
  Write-Host "TERMODEL_AI_APPARTAMENTO_FIN_THICKNESS_DEBUG_COMPLETED"
  Write-Host ($lines -join [Environment]::NewLine)
  if ($coverageChecked -ne 9 -or $coverageFailures -ne 0) {
    throw "FIN wall-thickness regression: checked=$coverageChecked failures=$coverageFailures"
  }
}
finally {
