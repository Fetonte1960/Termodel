...
// ?using Xbim.Ifc4.PresentationAppearance;
        ...
        public Coordinate3D? CentroPonte
{
    ...
    public partial class LeggiDxf : Component
    {
        ...
        private ListBox coordinateListBox;
        ...
        // Nuovo costruttore che accetta un Canvas
        public LeggiDxf(Canvas canvas, ListBox listbox, UtiDb utidbInstance, Modello modelloEdificio)
        {
...
📐 coordinate:
   ...
   - end:   X={parete.EndPoint.X} Y={parete.EndPoint.Y}", category: TermodelLog.LogCategory.colmi);
        ...
        public XbimVector3D Normalize(XbimVector3D vector)
        {
            double length = Math.Sqrt(vector.X * vector.X + vector.Y * vector.Y + vector.Z * vector.Z);
            if (length > 0)
            {
                return new XbimVector3D(vector.X / length, vector.Y / length, vector.Z / length);
            }
        ...
        Vector3D traslazione =  new Vector3D(0,0,0);
