...
if salvadb then
  ...
  FWriteln(FComDB,'       end;');
  FWriteln(FComDB,'  Nuovo:crea_'+Nometabella+'(T_'+Nometabella+');');
...
write_ln(FcreadbT ,'If Nome='''+Nometabella+''' then Crea_'+Nometabella+'(Dmtutti.T_'+Nometabella+');');
...
FWriteln(FUdb,'');
FWriteln(FUdb,'Type dbcom=(Init,chiudi,nuovo,apri,salva,leggi);');
...
if form1.cbnobde.checked  then FWriteln(FCreadbT,'uses Udbt,db,DBtablesQ;')
else FWriteln(FCreadbT,'uses Udbt,db,DBtables;');
...
FWriteln(fIoDati,'Uses DB,Varcarichi,udb,'+UsesDb+',Sysutils,math;');
...
else FWriteln(FUDataoutT,'uses Sysutils,Varcarichi,Math,db,Udb,CreaDbT,DBtables;');
...
FWriteln(FUdb,'    Procedure ComDB(action:DBCom;Par:string);');
FWriteln(FUdb,'  end;');
FWriteln(FUdb,'');
FWriteln(FUdb,'var');
...
FWriteln(FUdb,'');
FWriteln(FUdb,'Uses Udatalink;');
FWriteln(FUdb,'{$R *.dfm}');
FWriteln(FUdb,'');
FWriteln(FUdb,'{$I MappaDB}');
FWriteln(FUdb,'{$I SaveFile}');
FWriteln(FUdb,'Procedure TDMTutti.ComDB(action:DBCom;Par:string);');
FWriteln(FUdb,'Begin');
