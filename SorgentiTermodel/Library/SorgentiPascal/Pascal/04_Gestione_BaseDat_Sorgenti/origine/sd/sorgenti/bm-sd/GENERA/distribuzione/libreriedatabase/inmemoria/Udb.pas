...
uses
  SysUtils, Classes, DB, DBTables, kbmMemTable, DbtablesQ;
...
type
  TDM1 = class(TDataModule)
    ...
    TT2: TTableQ;
            ...
            procedure TT2NewRecord(DataSet: TDataSet);
...
uses
  ...
  SaveTxt_Orari(dm1.tt1,dm1.tt3,dm1.DataSource1);
  SaveTxt_finestre(dm1.tt1,dm1.tt3,dm1.DataSource1);
  writeCampo('#ENDFILE#');
  {$ELSE}
    writeCampo('Versione database ' + datacor);
  ...
  procedure TDM1.TT2NewRecord(DataSet: TDataSet);
  begin
    tt2.edit;
    if tt2.tablename[length(tt2.tablename)-2]='.' then
       InitDb(copy(tt2.tablename,1,length(tt2.tablename)-3))
    else InitDb(tt2.tablename)
  end;
