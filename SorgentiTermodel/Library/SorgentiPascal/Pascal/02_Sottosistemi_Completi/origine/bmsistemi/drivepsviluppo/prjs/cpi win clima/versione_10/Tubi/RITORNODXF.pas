...
procedure Disrit_can(nome_rete:string);
...
if fileexists(IncludeTrailingPathDelimiter(PercorsoDrive) +nome_rete+'.dsc') then
  begin
  assignfile(fdim,IncludeTrailingPathDelimiter(PercorsoDrive) +nome_rete+ '.dsc');
  try
    reset(fdim);
    ...
    begin
      ...
      'Q':Begin
          ...
          Testo_Dim(piano,float_to_str(x2,0),x1,y1,pi/2,'ESECUTIVO_RETE','7');
          end;
...
if (dati^[i]^.Term<>0)and(cisono_collettori{dis^[dati^[i]^.Ti]^.cl  approfondire }) then
  ...
  while j<>0 do
    ...
    if cl then
      ...
      { TODO : Vittorio: Quando z1 è diverso da z2 crea dei problemi.
      Aggiunto:  if dis^[j]^.z1=dis^[j]^.z2 }  ///
      if dis^[j]^.z1=dis^[j]^.z2 then linea_dim(Piano,x1,y1,x2,y2,'ESECUTIVO_RETE',Colore,TipoLINEA);
      ///
      Lineadeb(piano,xp2,yp2,x2,y2,j,'1');
      end;
...
Var i,j:Integer;
    ...
    Prx,pry:real;
begin
Spostx:=-ManRit;    //Vittorio modifica  ver 1.03
debug3d_on:=false;
is_rimando:=false;
X1p:=XPr;
Y1p:=YPr;
...
if Dis^[i]^.Coll then
  spostx:=0    //scalabilità dei collettori Diego 5/9/2011
  else
if spostx=0 then spostx:=-ManRit;
...
While Dis^[i]^.NLinea<>0 do    {-- Spostamento tronco --}
  begin
   ...
   { TODO :
Vittorio: Quando z1 è diverso da z2 crea dei problemi.
Aggiunto: if (Dis^[i]^.z1<>Dis^[i]^.z2)  then begin
  i:=Dis^[i]^.NLinea ;
  X1p:=Dis^[i]^.x1;
  Y1p:=Dis^[i]^.Y1+spy;
  end
  ...
  begin
Non è detto che funzioni sempre. Necessari + esempi per renderlo universale }
