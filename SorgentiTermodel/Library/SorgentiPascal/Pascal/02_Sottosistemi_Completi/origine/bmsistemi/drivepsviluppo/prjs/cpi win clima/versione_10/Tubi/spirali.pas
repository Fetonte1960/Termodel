...
with dis^[indln]^ do
 ...
 then Arco_dim(Piano,x2,y2,doppiop/2,dirr+pi,dirr,'ESECUTIVO_RETE','5','')
  ...
  angoli.ModiCord1(xp2,yp2,x1,y1,dirr,1);
  //modifica diego
  Centrotesto(xp2,yp2,Numero_terminale);
  Testo_Dim(piano,Numero_terminale,xp2,yp2,pi/2,'ESECUTIVO_RETE','3');
...
dist[199]:=10E6;
  ...
  with Vet_ln[k] do begin
   ...
   Calc_D(ln.x0,ln.y0,0,ln.x1,ln.y1,0,dir_parallela,dirz);  //evita le linee parallele
   dir_parallela:=dir_parallela-(int(dir_parallela/(2*pi))*2*pi);   //ver 1.17
...
if (K_prima<>k)  then begin    // esclude vicoli ciechi trovati nel precendente count
 ...
 if (res<>0) and (countiter=16) then//  and  (count=1)  and (k=34)  then
  ...
  K_min[k]:=0;
  if (res<>0) and (int(dir*2)<>int(dir_parallela*2))then  begin
